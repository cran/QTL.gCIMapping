QTL.gCIMapping<-function(file=NULL,fileFormat=NULL,fileICIMcov=NULL,Population=NULL,Model=NULL,WalkSpeed=NULL,
  ...
  if(is.character(file)==FALSE){
    result<-data.frame(Trait=c("grains.per.panicle","grains.per.panicle"),Chr=c(1,1),Position=c(5.9190,6.5310),
                       Effect.a=c(-3.9614,0),Effect.d=c(0,-2.5968),LOD=c(6.7336,3.3834),Left_marker=c("Bin43","Bin48"),
                       right_marker=c("Bin43","Bin48"),Var_Genet=c(7.8462,1.6859),r2=c(2.5144,0.5403),
                       Var_Error=c(101.202," "),Var_Phen=c(312.0546," "))
    return(result)  
  }else{
